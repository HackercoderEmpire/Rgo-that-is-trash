...
SettingsTab:CreateButton({
    Name = "HD Admin",
    Callback = function()
        local assetId = 4866692557 -- Asset ID for the HD Admin script
        local success, result = pcall(function()
            local asset = game:GetService("InsertService"):LoadAsset(assetId)
            asset.Parent = game.Workspace  -- Or wherever you'd like to parent it
        end)
        
        if success then
            print("HD Admin loaded successfully!")
        else
            warn("Failed to load HD Admin: " .. tostring(result))
        end
    end
})
local mainButton1 = SettingsTab:CreateButton({
    Name = "CHAT HACKS Server Side",
    Description = "",
    Callback = function()
        local function customScript()
--[[
